...
if (gulpVersion == 4) {
	...
	gulp.task('watch', function() {
		...
		gulp.watch('smartgrid.js', gulp.series('grid', 'styles'));
		gmWatch && gulp.watch('app/img/_src/**/*', gulp.parallel('img')); // GraphicsMagick watching image sources if allowed.
	});
	gmWatch ? gulp.task('default', gulp.series('grid', gulp.parallel('styles', 'scripts', 'browser-sync', 'watch')))
					: gulp.task('default', gulp.series('grid', gulp.parallel('styles', 'scripts', 'browser-sync', 'watch')));
